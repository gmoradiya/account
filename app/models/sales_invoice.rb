class SalesInvoice < ApplicationRecord
  ...
  accepts_nested_attributes_for :sales_inventories, allow_destroy: true
  ...
  def generate_bill_no
    last_invoice = SalesInvoice.order(created_at: :desc).first
    last_number = last_invoice&.bill_no&.split("-")&.last.to_i || 0
    self.bill_no = "INV-#{(last_number + 1).to_s.rjust(5, "0")}"  # Format: INV-00001
  end
